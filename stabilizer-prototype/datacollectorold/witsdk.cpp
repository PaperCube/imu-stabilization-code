...
namespace witsdk {
    ...
    static SerialWrite p_WitSerialWriteFunc = nullptr;
    static WitI2cWrite p_WitI2cWriteFunc = nullptr;
    static WitI2cRead p_WitI2cReadFunc = nullptr;
    static CanWrite p_WitCanWriteFunc = nullptr;
    static RegUpdateCb p_WitRegUpdateCbFunc = nullptr;
    static DelaymsCb p_WitDelaymsFunc = nullptr;
...
#define FuncR 0x03
    ...
    static uint16_t __CRC16(const uint8_t *puchMsg, uint16_t usDataLen) {
    ...
    /**
     * Original func name: CopeWitData
     *
     * @param ucIndex (conjectured) type of data
     * @param p_data (conjectured) pointer to data
     * @param uiLen (conjectured) length of data
     */
    static void WitHandleData(uint8_t ucIndex, uint16_t *p_data, uint32_t uiLen) {
        ...
        // Optimized out: condition always true
//        if (uiReg1Len) {
...
//        }
    ...
    void WitSerialDataIn(uint8_t ucData) {
        ...
        if (p_WitRegUpdateCbFunc == nullptr)return;
        ...
        switch (s_uiProtoclo) {
            ...
            case WIT_PROTOCOL_NORMAL:
                ...
                if (s_uiWitDataCnt >= 11) {
                    ...
                    WitHandleData(s_ucWitDataBuff[1], usData, 4);
                    s_uiWitDataCnt = 0;
                }
                break;
            ...
            case WIT_PROTOCOL_MODBUS:
                if (s_uiWitDataCnt > 2) {
                    ...
                    usTemp =
                            ((uint16_t) s_ucWitDataBuff[s_uiWitDataCnt - 2] << 8) | s_ucWitDataBuff[s_uiWitDataCnt - 1];
    ...
    void WitCanDataIn(uint8_t ucData[8], uint8_t ucLen) {
        ...
        switch (s_uiProtoclo) {
            ...
            case WIT_PROTOCOL_CAN:
                ...
                WitHandleData(ucData[1], usData, 3);
                break;
    ...
    int32_t WitWriteReg(uint32_t uiReg, uint16_t usData) {
        ...
        switch (s_uiProtoclo) {
            ...
            case WIT_PROTOCOL_NORMAL:
                if (p_WitSerialWriteFunc == nullptr)return WIT_HAL_EMPTY;
                ucBuff[0] = 0xFF;
                ucBuff[1] = 0xAA;
                ucBuff[2] = uiReg & 0xFF;
            ...
            case WIT_PROTOCOL_905x_CAN:
            case WIT_PROTOCOL_CAN:
                if (p_WitCanWriteFunc == nullptr)return WIT_HAL_EMPTY;
                ucBuff[0] = 0xFF;
                ucBuff[1] = 0xAA;
                ucBuff[2] = uiReg & 0xFF;
                ucBuff[3] = usData & 0xff;
                ...
                break;
            case WIT_PROTOCOL_I2C:
                if (p_WitI2cWriteFunc == nullptr)return WIT_HAL_EMPTY;
    ...
    void WitDestroy() { // original signature: WitDeInit
        p_WitSerialWriteFunc = nullptr;
        p_WitI2cWriteFunc = nullptr;
        p_WitI2cReadFunc = nullptr;
        p_WitCanWriteFunc = nullptr;
        p_WitRegUpdateCbFunc = nullptr;
        p_WitDelaymsFunc = nullptr;
        s_ucAddr = 0xff;
        s_uiWitDataCnt = 0;
        s_uiProtoclo = 0;
    }
